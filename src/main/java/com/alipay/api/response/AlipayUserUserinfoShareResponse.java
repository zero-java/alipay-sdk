 ...
 */
@ApiModel(description = " ALIPAY API: alipay.user.userinfo.share response.")
public class AlipayUserUserinfoShareResponse extends AlipayResponse {
     ...
     * 区域编码，暂时不返回值
     */
    @ApiModelProperty(notes = " 区域编码，暂时不返回值")
     ...
     * UNKNOWN ---- 降级、或查询超时
     */
    @ApiModelProperty(notes = " 【注意】当is_balance_frozen为“F”时，改字段不会返回.CTU ---- CTU冻结，允许用户开启ALIBABA ---- ALIBABA冻结，允许用户开启SERVER ---- 后台冻结，允许用户开启USER ----  用户冻结 CTU_N---- CTU冻结，不允许用户开启ALIBABA_N ---- ALIBABA冻结，不允许用户开启SERVER_N ---- 后台冻结，不允许用户开启UNKNOWN ---- 降级、或查询超时")
     ...
     * 10其它证件
     */
    @ApiModelProperty(notes = " 0:身份证1:护照2:军官证3:士兵证4:回乡证5:临时身份证6:户口簿7:警官证8:台胞证9:营业执照10其它证件")
     ...
     * 是否默认收货地址，暂时不返回值
     */
    @ApiModelProperty(notes = " 是否默认收货地址，暂时不返回值")
     ...
     * 收货人地址列表
     */
    @ApiModelProperty(notes = " 收货人地址列表")
     ...
     * 用户支付宝账号绑定的邮箱地址
     */
    @ApiModelProperty(notes = " 用户支付宝账号绑定的邮箱地址")
    @ApiField("email")
     ...
     */
    @ApiModelProperty(notes = " 姓氏，取的是realName中的首个字符，对非中文、中文复姓支持较差。")
     ...
     */
    @ApiModelProperty(notes = " 公司名称（用户类型是公司类型时公司名称才有此字段）。")
    @ApiField("firm_name")
     ...
     * T--被冻结；F--未冻结
     */
    @ApiModelProperty(notes = " T--被冻结；F--未冻结")
     ...
     * T为是银行卡认证，F为非银行卡认证。
     */
    @ApiModelProperty(notes = " T为是银行卡认证，F为非银行卡认证。")
     ...
     */
    @ApiModelProperty(notes = " T：表示A类实名认证；F：表示非A类实名认证")
     ...
     * T为是身份证认证，F为非身份证认证。
     */
    @ApiModelProperty(notes = " T为是身份证认证，F为非身份证认证。")
    @ApiField("is_id_auth")
     ...
     * T为通过营业执照认证，F为没有通过
     */
    @ApiModelProperty(notes = " T为通过营业执照认证，F为没有通过")
     ...
     * T为是手机认证，F为非手机认证。
     */
    @ApiModelProperty(notes = " T为是手机认证，F为非手机认证。")
     ...
     */
    @ApiModelProperty(notes = " 脱敏登录号。脱敏后的登录号，部分字符被遮盖.")
     ...
     * 用户的真实姓名。
     */
    @ApiModelProperty(notes = " 用户的真实姓名。")
    @ApiField("real_name")
     ...
     * 生日的月和日，MMdd格式
     */
    @ApiModelProperty(notes = " 生日的月和日，MMdd格式")
     ...
     * 用户的userId
     */
    @ApiModelProperty(notes = " 用户的userId")
    @ApiField("user_id")
